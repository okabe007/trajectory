...
#         if distance > radius + limit:
#             # dropの外に出た → 吸着を開始
#             new_stick_status = * constants["sample_rate_hz"]
#             return IOStatus.ON_POLYGON, new_stick_status
#         else:
...
#######
    ...
    def handle_ON_POLYGON(self, j, i, status, base_pos, temp_pos, intersection, stick_status, constants):
        self.trajectory[j, i] = temp_pos
    ...
    def handle_sphere_out(self, j, i, status, base_pos, temp_pos, intersection, stick_status, constants):
        ...
        return IOStatus.ON_POLYGON, base_pos, temp_pos, last_vec, stick_status
    ...
    def bottom_edge_mode(self, base_position, last_vec, stick_status, constants):
        ...
        後者コードを反映して修正。 ON_POLYGON から呼ばれる。
        """
    ...
    def draw_motion_area(self, shape, axes, constants):
        if shape == 'spot':
            ...
            for ax in axes:
                ...
                ax.set_aspect('equal')
                circle = plt.Circle((0, 0), drop_r, color='blue', alpha=0.1)
                ax.add_patch(circle)
...
class SpermTrajectoryVisualizer:
    def __init__(self, simulation):
        self.simulation = simulation
        self.constants = simulation.constants
        self.egg_center = np.array([
            self.constants.get("egg_x", 0.0),
            self.constants.get("egg_y", 0.0),
            self.constants.get("egg_z", 0.0),
        ])
        self.egg_radius = self.constants.get("gamete_r", 0.1)

    def _setup_3d_axes(self):
        fig = plt.figure(figsize=(10, 6))
        ax = fig.add_subplot(111, projection='3d')
        x_min, x_max, y_min, y_max, z_min, z_max = get_limits(self.constants)
        z_min = min(z_min, self.egg_center[2] - self.egg_radius)
        z_max = max(z_max, self.egg_center[2] + self.egg_radius)

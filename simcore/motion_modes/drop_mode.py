...
def drop_polygon_move(
    ...
    - stick_status == 1 : 球体内方向にやや浮かせる
    - stick_status == 0 : 自由運動（ON_POLYGON終了）

    Returns:
        new_temp_position: 次の位置
        new_last_vec: 新しい方向ベクトル
        new_stick_status: 残りステップ数
        next_state: IOStatus（ON_POLYGON or INSIDE）
    """
    ...
    next_state = IOStatus.INSIDE if new_stick_status <= 0 else IOStatus.ON_POLYGON

...
def draw_medium(ax, constants: dict):
    ...
    if shape == "spot":
        ...
        bottom_h = constants["spot_bottom_height"]
        center_z = 0.0 + spot_r
        u = np.linspace(0, 2 * np.pi, 50)
        v = np.linspace(np.arccos((spot_r - bottom_h) / spot_r), np.pi, 50)
        x = spot_r * np.outer(np.cos(u), np.sin(v))
        y = spot_r * np.outer(np.sin(u), np.sin(v))
        z = spot_r * np.outer(np.ones_like(u), np.cos(v)) + center_z
        ax.plot_surface(x, y, z, alpha=0.1, edgecolor='gray', linewidth=0.5)
